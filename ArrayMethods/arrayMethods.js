...
console.log(filteredMappedUsers);

//Ex4
let cNamedUsers = users
  .map((user) => {
    return user.name;
  })
  .filter((name) => {
...
console.log(cNamedUsers);

//Ex5
let areUsersFromSouthChristy = users.every((user) =>
   user.address.city === "South Christy");

console.log(areUsersFromSouthChristy)

//Ex6
let userInApt950 = users.find(user => user.address.suite === "Apt. 950")

console.log(userInApt950.company.name)
...
users.forEach(user => console.log(userDescription(user)))
...
const convertGrade = function(grade){
...
let itemCosts = cartItems.reduce((total, item) =>{
    let tax = taxRates[item.category]
    return total + item.price * item.quantity + item.price * tax
},0)
